...
# The installer is intentionally NOT part of the main solution build.
# It embeds the CP DLL while the CP project is building, so parallel solution builds
# can lock the payload DLL. Build the main solution first, then the installer.
...
foreach ($payload in $payloads) {
    if (-not (Test-Path $payload)) { throw "Required payload is missing: $payload" }
}

Write-Host "2/2 Building GCL-Installer Release x64..."
& $msbuild $installerSolution /t:Rebuild /restore /m /p:Configuration=Release /p:Platform=x64
if ($LASTEXITCODE -ne 0) { throw "Installer build failed." }

$out = Join-Path $root "src\GenericCardLogon.Installer\bin\x64\Release\net48\GCL-Installer.exe"
if (-not (Test-Path $out)) { throw "Installer EXE was not produced: $out" }
