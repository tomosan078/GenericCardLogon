...
namespace GenericCardLogon.Service
{
    ...
    internal sealed class SonyFelicaPolling : IDisposable
    {
        ...
        public byte[] PollExpressIdm()
        {
            ...
            // A second single-slot poll helps when the first RF cycle wakes the
            // mobile FeliCa endpoint. Keep a small quiet gap so we do not hammer
            // the watch with back-to-back reader activations.
            Thread.Sleep(60);
            ...
            // Do not perform another immediate 0x00 poll here. The caller owns
            // the retry budget and will start a fresh RF cycle after a quiet gap.
            return null;
        }
